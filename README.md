...
```sh
node --test tests/engine.test.cjs tests/page.test.cjs tests/special-rules.test.cjs
```
...
五種食物各有底色；四消使用強化食物圖與方向標記，章魚燒和棉花糖各有獨立外觀。拖曳到相鄰格時，特殊交換會預覽交換後第一波清除範圍與同類目標。棋盤下方可切換「依系統偏好／減少動態／完整效果」，設定會保存在本機；減少動態仍保留生成、方向與發動標記。

| 配對方式 | 生成物與效果 |
| --- | --- |
| 橫向四消 | ↔ 強化食物，清除所在整列 |
| 縱向四消 | ↕ 強化食物，清除所在整欄 |
| T／L／十字：同類的橫、縱各至少三格並有交點 | 章魚燒炸彈，清除周圍 3×3 |
| 直線五消以上 | 彩虹棉花糖，清除交換食物的同類 |

特殊與任意相鄰方塊交換即可發動，並觸發範圍內其他特殊；直線會穿過特殊繼續清到邊界。強化食物保留種類並可配對，章魚燒與棉花糖不帶食物種類。同一組交疊配對只產生一顆，優先順序為直線五消、交叉型、四消；新生成特殊當波保留，既有特殊不會被覆蓋。

| 特殊交換 | 結果 |
| --- | --- |
| 強化＋強化、強化＋章魚燒、雙章魚燒 | 各自在交換後位置發動，重疊格只清一次 |
| 棉花糖＋強化食物 | 清該種類並觸發既有同類強化食物 |
| 棉花糖＋章魚燒 | 清盤上數量最多的普通食物種類，並引爆章魚燒 |
| 雙棉花糖 | 全盤清除 |

棉花糖被其他特殊波及時也採普通食物多數規則；平手依珍奶、地瓜球、臭豆腐、雞排、烤玉米順序。選定後仍會清除並觸發該種類的強化食物。規則不會把整批普通食物轉成特殊。
...
可用 `python optimize_assets.py` 重建（需要 Pillow）。素材尺寸、來源雜湊、深淺背景與 40px 縮圖預覽見 [素材檢查說明](tools/asset-review/README.md)。正式遊戲已接入直線消除與 T／L／十字章魚燒；互動教學共用正式引擎的改造留在 P3。
